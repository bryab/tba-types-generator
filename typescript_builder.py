...
def _convert_single_type(type_name):
    ...
    type_name = type_name.replace("virtual", "").replace("static", "") # FIXME: May want static (but is not part of type in TS)
...
def write_jsdoc(f, obj):
    ...
    for param in obj.get('params',[]):
        ...
        if 'default' in param:
            f.write(" [{0}={1}]".format(param['name'], convert_value(param['default'])))
        else:
...
def build_type(prop):
    if 'object_schema' in prop:
        ...
        for field_data in prop['object_schema']:
            ...
            type_str += "\n{}:{}".format(field_data['name'], convert_type(field_data['type']))
        type_str += "}"
    else:
...
def write_ts_from_class(cls: dict, f: typing.TextIO):
    ...
    is_static = not 'parent' in cls or cls['parent'] in ['GlobalObject', 'BAPP_SpecialFolders']
    static_str = ""
    if is_static:
        static_str = "static "
    ...
    if is_module:
        ...
        if cls.get('parent', None):
            f.write('\ndeclare class {} extends {} {{'.format(cls['name'], cls['parent']))
        else:
    ...
    for slot in cls['slots']:
        ...
        if is_module:
            f.write('\n{prefix}function {name} ({sig}): {type};\n'.format(prefix=prefix, name=slot['name'], sig=sig, type=type_str))
        else:
            if slot['name'] == cls['name']:
                f.write('\nconstructor ({sig});\n'.format(name=slot['name'], sig=sig, type=type_str))
            else:
                f.write('\n{prefix}public {static}{name} ({sig}): {type};\n'.format(prefix=prefix, static=static_str, name=slot['name'], sig=sig, type=type_str))
    for signal in cls.get('signals', []):
        ...
        f.write('\n{prefix}public {name}: {type};\n'.format(prefix=prefix, name=signal['name'], type=type_str))
    for prop in cls['props']:
        ...
        if is_module:
            f.write('\n{prefix}var {name}: {type};\n'.format(prefix=prefix, name=prop['name'], type=type_str))
        else:
            f.write('\n{prefix}{static}{name}: {type};\n'.format(prefix=prefix, static=static_str, name=prop['name'], type=type_str))
    f.write("\n}\n\n")
