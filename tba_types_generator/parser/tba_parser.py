...
def _parse_enum_div(div: bs4.element.Tag) -> Dict:
    ...
    enum_data.update(_parse_signature_table(
        div.find('table', {'class': 'memname'})))
...
def _parse_function_div(div: bs4.element.Tag) -> Dict:
    ...
    func_data.update(_parse_signature_table(
        div.find('table', {'class': 'memname'})))
...
def _find_class_name(soup: BeautifulSoup) -> str:
    ...
    return div.find(text=True).strip().replace("Class Reference", "").replace("Namespace Reference", "").strip()
...
def _find_class_desc(soup: BeautifulSoup) -> Tuple[str, str]:
    desc = ""
    example = ""
    if a := soup.find("a", attrs={
        "id": "details"
    }):
        if txt := a.find_next("div", attrs={
            "class": "textblock"
        }):
            desc_lines = []
